    ...
    with open("./results/books.csv","a+",encoding="utf8",newline="")as csvfile:
        ...
        try:
            for url in urls:
                ...
                try:
                    tar = request_get(book_url)
                    #print(tar)

                except Exception as e:
                    print(e)
                    left_urls.remove(book_url)
                    continue
                ...
                #print(str(current_num)+"/"+str(total_num)+"： "+str(score)+str(title))
                print("{0}/{1}: {2}  {3}".format(current_num,total_num,score,str(title)))
                try:
            ...
            dataframe = pd.DataFrame({'url': list(left_urls)})
            dataframe.to_csv("./results/working_urls.csv", index=False, sep=',')
...
def read_urls():
    ...
    if  os.path.exists("./results/working_urls.csv"):
        tags_col_all = pd.read_csv("./results/working_urls.csv")
    working_num=len(tags_col_all['url'].values.tolist())
    if not os.path.exists("./results/books.csv"):
        ...
        print("This excuting plans to downing {0} ".format(working_num))
        print("Have Faild {0}".format(total_num-current_num-working_num))
