...
def get_pic_urls(table):
    sql_read_pic_urls = f"select `id`, `pic_url` from `{table}` where `zimg_url` is NULL and `pic_url` is not NULL;"
...
zimg_base_url = "http://107.191.52.220:4869/"
upload_image_url = "http://107.191.52.220:4869/upload"
interval = 2
...
def download_store_images(table, id_2_pic_urls):
    ...
    for t_id in id_2_pic_urls:
        ...
        try:
            ...
            print(ex)
            continue
        print("%d Done; zimg_url: %s" % (t_id, zimg_url))
        # time.sleep(interval)
